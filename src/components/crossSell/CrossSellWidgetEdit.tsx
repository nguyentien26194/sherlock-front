...
export default function CrossSellWidgetEdit({ widgetId }: Props) {
  ...
  const getCrossSellWidget = async () => {
    const response = await axiosPrivate.get(`/api/cross-sell-widgets/${widgetId}/?shop_id=` + cookies.shopId);

    if (response?.status === 200) {
      setCrossSellName(response.data.name);
      const crossSellProducts = response.data.detailed_products.map((product: ProductModel) => {
        return {
          id: product.id,
      ...
      setSelectedCrossSellProducts(crossSellProducts);
      if (response.data.selected_all_products) {
  ...
  const handleEditCrossSellWidget = async () => {
    setLoading(true);
    const response = await axiosPrivate.put(`/api/cross-sell-widgets/${widgetId}/`, {
      name: crossSellName,
      shop: cookies.shopId,
          ...
          <Grid item xs={12} md={12} lg={5}>
            ...
            >
              <Grid container direction="column">
                ...
                >
                  <Button variant="contained" onClick={() => handleEditCrossSellWidget()} disabled={loading}>
                    Edit Cross-sell
                  </Button>
                  {loading && (
