...
import { ALL_PRODUCTS, FIXED_AMOUNT, PERCENTAGE, SPECIFIC_PRODUCTS } from '../../Constants';
...
export default function UpsellWidgetEdit({ widgetId }: Props) {
  ...
  const getUpsellWidget = async () => {
    const response = await axiosPrivate.get(`/api/upsell-widgets/${widgetId}/?shop_id=` + cookies.shopId);

    if (response?.status === 200) {
      setUpsellName(response.data.name);
      setUpsellTitle(response.data.offer_name);
      setSelectedUpsellProduct([
        {
          id: response.data.detailed_product.id,
          cms_product_id: response.data.detailed_product.cms_product_id,
          title: response.data.detailed_product.title,
          shortened_title: response.data.detailed_product.shortened_title,
          image_url: response.data.detailed_product.image_url,
          image_urls: response.data.detailed_product.image_urls,
          price: response.data.detailed_product.price,
          inventory_quantity: response.data.detailed_product.inventory_quantity,
        },
      ]);
      const triggerProducts = response.data.trigger_products.map((product: ProductModel) => {
        return {
          id: product.id,
      ...
      setSelectedTriggerProducts(triggerProducts);
      if (response.data.selected_all_trigger_products) {
        setTriggerType(ALL_PRODUCTS);
      } else {
        setTriggerType(SPECIFIC_PRODUCTS);
      }
      if (response.data.discount_value > 0) {
        setActiveDiscount(true);
        setDiscountValue(response.data.discount_value);
        setDiscountType(response.data.discount_value_type);
      }
    }
    setLoading(false);
  };
  ...
  const handleEditUpsell = async () => {
    if (selectedUpsellProduct.length > 0) {
      setLoading(true);
      const response = await axiosPrivate.put(`/api/upsell-widgets/${widgetId}/`, {
        name: upsellName,
          ...
          <Grid item xs={12} md={12} lg={5}>
            ...
            >
              <Grid container direction="column">
                ...
                >
                  <FormControl variant="outlined" sx={{ mr: 2, width: '28%' }} disabled={!activeDiscount}>
                ...
                >
                  <Button variant="contained" onClick={() => handleEditUpsell()} disabled={loading}>
                    Edit Upsell
                  </Button>
                  {loading && (
